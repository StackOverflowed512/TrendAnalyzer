...
from app.schemas.prompt import PromptGenerationRequest, StoryboardResponse
...
def save_trend(trend: TrendSaveRequest, db: Session = Depends(get_db)):
    ...
    if trend.platform == "youtube":
        ...
        url = (
            f"https://reddit.com{trend.id}"
            if trend.id.startswith("/")
            else f"https://reddit.com/{trend.id}"
        )
    else:
...
def generate_prompt(
    video_id: int, request: PromptGenerationRequest, db: Session = Depends(get_db)
):
    video = video_repo.get(db, id=video_id)
    ...
    trending_video_data = {"title": video.title, "description": video.description}
    prompt = service.generate(
        template=video.analysis_json, trending_video=trending_video_data
    )
